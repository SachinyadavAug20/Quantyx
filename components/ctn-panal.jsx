...
const Ctnpanal = ({ editable, screentext, setScreentext, cursorPosition, setCursorPosition, insertAtCursor }) => {
    ...
    return (
        <motion.div variants={containerVariants} initial="hidden" animate="visible" className="grid grid-cols-6 grid-rows-3">
            <div>
            ...
            <div>
                <motion.button variants={itemVariants} whileHover={{ opacity: 1 }} transition={{ duration: 0.4, type: "spring", ease: "easeOut" }} whileTap={{ scale: 1.15, opacity: 1 }} onClick={() => { editable ? insertAtCursor(' rad') : "" }} type="button" className="bg-orange-600 text-white cursor-pointer px-3.5 text-xl py-1 border-2 border-slate-100 rounded-xl mx-1 border-b-[6px] my-2">rad</motion.button>
            </div>
            <div>
                <motion.button variants={itemVariants} whileHover={{ opacity: 1 }} transition={{ duration: 0.4, type: "spring", ease: "easeOut" }} whileTap={{ scale: 1.15, opacity: 1 }} onClick={() => { editable ? insertAtCursor('pi') : "" }} type="button" className="bg-orange-600 text-white cursor-pointer px-6 text-xl py-1 border-2 border-slate-100 rounded-xl mx-1 border-b-[6px] my-2">π</motion.button>
            </div>
            <div>
                <motion.button variants={itemVariants} whileHover={{ opacity: 1 }} transition={{ duration: 0.4, type: "spring", ease: "easeOut" }} whileTap={{ scale: 1.15, opacity: 1 }} onClick={() => { editable ? insertAtCursor('sqrt(') : "" }} type="button" className="bg-orange-600 text-white cursor-pointer px-5 text-xl py-1 border-2 border-slate-100 rounded-xl mx-1 border-b-[6px] my-2">√</motion.button>
            </div>
            <div>
                <motion.button variants={itemVariants} whileHover={{ opacity: 1 }} transition={{ duration: 0.4, type: "spring", ease: "easeOut" }} whileTap={{ scale: 1.15, opacity: 1 }} onClick={() => { editable ? insertAtCursor('^') : "" }} type="button" className="bg-orange-600 text-white cursor-pointer px-5 text-xl py-1 border-2 border-slate-100 rounded-xl mx-1 border-b-[6px] my-2">^</motion.button>
            </div>
        </motion.div>
    )
}
